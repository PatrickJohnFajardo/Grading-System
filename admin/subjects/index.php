...
if (!empty($search)) {
    $searchQuery = " WHERE code LIKE ? OR title LIKE ?";
    $searchTerm = "%{$search}%";
    $params = [$searchTerm, $searchTerm];
}

// Get total count
$countSql = "SELECT COUNT(*) as count FROM subjects" . $searchQuery;
$totalRecords = queryOne($countSql, $params)['count'];
$totalPages = ceil($totalRecords / $perPage);

// Get subjects with semester info
$sql = "SELECT s.*, sem.name as semester_name 
        FROM subjects s 
        LEFT JOIN semesters sem ON s.semester_id = sem.id" 
        . $searchQuery . " ORDER BY s.code LIMIT ? OFFSET ?";
$params[] = $perPage;
$params[] = $offset;
$subjects = query($sql, $params);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted">Total: <?php echo $totalRecords; ?> subjects</p>
    </div>
...
<div class="card">
    ...
    <div class="card-body">
        <?php if (empty($subjects)): ?>
            <p class="text-center text-muted py-4">No subjects found.</p>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($subjects as $subject): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($subject['semester_name'] ?? 'N/A'); ?></td>
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        ...
                                        <button type="button" class="btn btn-danger" 
                                                onclick="confirmDelete(<?php echo $subject['id']; ?>, '<?php echo htmlspecialchars($subject['code']); ?>')"
                                                title="Delete">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Are you sure you want to delete subject <strong id="deleteSubjectCode"></strong>?
